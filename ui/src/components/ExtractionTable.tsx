...
function confidenceClass(confidence: number | null, hasExtraction: boolean) {
  if (!hasExtraction) return "bg-neutral-100 text-neutral-500";
        ...
        <table className="min-w-full divide-y divide-neutral-200 text-sm">
          ...
          <tbody className="divide-y divide-neutral-100">
            {REQUIRED_FIELDS.map((fieldName) => {
              ...
              const valueLabel = !hasExtraction
                ? isPending
                  ? "Extracting..."
                  : "-"
                : field?.is_present
              ...
              const confidenceLabel = !hasExtraction
                ? "-"
                : confidence === null
                  ? "missing"
              ...
              return (
                <tr key={fieldName}>
                  ...
                  <td className="py-3 pr-4">{valueLabel}</td>
                  <td className="py-3 pr-4">
                    <span
                      className={`rounded-full px-2 py-1 text-xs ${confidenceClass(
                        confidence,
                        hasExtraction
                      )}`}
                    >
